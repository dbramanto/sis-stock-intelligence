...
def map_stage2_output(records: Iterable[Mapping[str, Any]], analysis_as_of: str,
    ...
    """Translate frozen Stage 2 output into P1 evidence contracts.

    This mapper is intentionally non-analytical: it does not recalculate scores,
    eligibility, confidence, priorities, or missing-data decisions.
    """
    rows = list(records)
    symbols = []
    evidence = []
    ...
    for i, rec in enumerate(rows):
        if not isinstance(rec, Mapping):
            diagnostics.append(f"ROW_{i}:INVALID_RECORD")
            continue
        symbol = _norm_symbol(rec.get("symbol"))
        if not symbol:
            diagnostics.append(f"ROW_{i}:MISSING_SYMBOL")
            continue
        if symbol in seen:
            diagnostics.append(f"{symbol}:DUPLICATE_STAGE2_SYMBOL")
            continue
        seen.add(symbol)

        state = _stage2_state(rec)
        if state is None:
            diagnostics.append(f"{symbol}:INVALID_STAGE2_STATE")
            continue
        if state not in allowed_states:
            excluded.append((symbol, f"STATE_NOT_IN_SCOPE:{state.value}"))
            continue
        symbols.append(symbol)

        # Preserve Stage2 state exactly as evidence.
        ...
        # Preserve known Stage2 block scores/states without recomputation.
        ...
        # Preserve horizon outputs as screening evidence only; no decision interpretation.
