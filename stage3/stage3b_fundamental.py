...
def evaluate_fundamental(s,analysis_date="2026-09-17"):
    ...
    internal_mode="S1_CANONICAL_INTERNAL" in str(source)
    confidence_fields=("npm","roic","eps_growth","revenue_growth","debt_equity","fscore","asset_turnover","gross_margin") if internal_mode else NUMERIC_FIELDS
    conf_cov=_coverage(e,confidence_fields)
    dq={"source":source,"as_of_date":meta.get("as_of_date"),"freshness":fresh,"age_days":age,
        "coverage":round(_coverage(e,NUMERIC_FIELDS),4),"confidence_coverage":round(conf_cov,4),
    ...
    conf=_confidence(conf_cov,fresh)
    def cconf(state): return "INSUFFICIENT" if state==NOT_EVALUATED else conf
    components={
      "profitability":result(profitability,[f"npm={npm}",f"roic={roic}"],cconf(profitability)),
      "growth":result(growth,[f"eps_growth={eps}",f"revenue_growth={rev}"],cconf(growth)),
      "efficiency":result(efficiency,[f"asset_turnover={ato}",f"gross_margin={gm}"],cconf(efficiency)),
      "financial_strength":result(financial_strength,[f"debt_equity={de}",f"fscore={fs}"],cconf(financial_strength)),
      "earnings_quality":result(earnings_quality,[f"gross_margin_3y_avg={gm3}",f"roic_3y_avg={roic3}"],cconf(earnings_quality)),
      "durability":result(durability,[f"gross_margin={gm}",f"gross_margin_3y_avg={gm3}",f"roic_3y_avg={roic3}"],cconf(durability)),
      "capital_allocation":result(capital_allocation,[f"share_count_growth={scg}",f"reinvestment_rate={rr}",f"dividend_payout={dp}"],cconf(capital_allocation)),
