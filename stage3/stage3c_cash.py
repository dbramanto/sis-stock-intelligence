...
def evaluate_cash(s,analysis_date="2026-09-17"):
    ...
    confidence_fields=("ocf_ttm","fcf_ttm","net_income_ttm","capex_ttm","revenue_ttm","cash_balance","short_term_debt") if internal_mode else FIELDS
    conf_cov=sum(1 for k in confidence_fields if _num(e.get(k)))/len(confidence_fields)
    dq={"source":source,"as_of_date":meta.get("as_of_date"),
        "freshness":fresh,"age_days":age,"coverage":round(cov,4),"confidence_coverage":round(conf_cov,4),
