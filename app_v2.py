...
with main:
    with st.container(border=True):
        ...
        valid_batches = len(parsed); expected_total = len(parsed[1]) if 1 in parsed else 0
        ...
        st.caption("🔒 Snapshot resmi hanya dibuat setelah Stage 1, Stage 2, Stage 3, dan seluruh validation guard PASS.")
        ...
        if s1.get("status") != "PASS":
            ...
            if s2.status != "PASS":
                ...
                if s3.get("status") != "COMPLETE":
                    ...
                    st.write("4/4 · Menyimpan snapshot tervalidasi")
...
with st.container(border=True):
    st.markdown('<div class="sectionhead"><h3>Alur Proses Background</h3><span class="eyebrow">Fail Closed</span></div><div class="flow"><div><b>1 · Parsing</b>Baca B1–B11</div><div><b>2 · Validasi</b>Guard & konsistensi</div><div><b>3 · Analisis</b>Stage 1 → 2 → 3</div><div><b>4 · Snapshot</b>Simpan bila PASS</div><div><b>5 · Hasil</b>Siap untuk D2</div></div>', unsafe_allow_html=True)
